...
export function NoteEditor() {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Image.configure({ inline: true, allowBase64: true, HTMLAttributes: { class: 'rounded-3xl shadow-[0_20px_50px_rgb(0,0,0,0.15)] my-10 mx-auto max-w-[95%] border border-white/10 transition-all hover:scale-[1.02] duration-500' } }),
      Link.configure({ openOnClick: false, HTMLAttributes: { class: 'text-indigo-400 hover:text-indigo-300 font-semibold underline decoration-indigo-500/30 hover:decoration-indigo-400 underline-offset-4 transition-colors' } })
    ],
    content: note?.content ?? '',
    onUpdate: ({ editor: currentEditor }) => { if (note) updateNote(note.id, { content: currentEditor.getHTML() }); },
    editorProps: {
      attributes: { class: `prose prose-2xl max-w-none focus:outline-none min-h-[60vh] pb-64 pt-8 leading-[1.8] font-sans selection:bg-indigo-500/30 selection:text-indigo-200 ${activeTheme === 'dim' ? 'prose-invert' : ''}` },
    },
  }, [note?.id, activeTheme]);
  ...
  const handleAI = async (action: AIAction) => {
    ...
    try {
      ...
      if (action === 'summarize') {
        ...
        if (points.length > 0) {
          const summaryHtml = `<div style="background: rgba(255,255,255,0.03); backdrop-filter: blur(24px); padding:40px; border-radius:32px; margin: 40px 0; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 40px 80px -20px rgba(0,0,0,0.5);">
  ...
  const insertGeneratedAsset = () => {
    ...
    const resultHtml = `<div style="padding: 4px; background: linear-gradient(135deg, rgba(99,102,241,0.1), rgba(168,85,247,0.1)); border-radius: 32px; margin: 40px 0; box-shadow: 0 30px 60px -15px rgba(0,0,0,0.5);">
      <div style="background: rgba(20,20,20,0.8); backdrop-filter: blur(20px); border-radius: 28px; padding: 32px; border: 1px solid rgba(255,255,255,0.05);">
        ...
        ${isImage ? `<img src="${escapeHtml(generatedAsset.assetUrl)}" style="border-radius: 20px; width: 100%; box-shadow: 0 20px 40px -10px rgba(0,0,0,0.5);" />` : `<a href="${escapeHtml(generatedAsset.assetUrl)}" target="_blank" style="display: inline-block; padding: 12px 24px; background: #6366f1; color: white; border-radius: 16px; text-decoration: none; font-weight: bold;">View 3D Asset</a>`}
      </div>
    </div>`;
  ...
  if (!note) {
    return (
      <div className={`flex-1 flex flex-col items-center justify-center relative overflow-hidden ${THEMES[activeTheme]}`}>  
        ...
        <button onClick={() => selectNote(addNote({ topicId: selectedTopicId, subjectId: selectedSubjectId, notebookId: selectedNotebookId }).id)} className="px-10 py-5 bg-white text-black font-black rounded-full shadow-[0_20px_40px_-10px_rgba(255,255,255,0.3)] hover:scale-105 hover:shadow-[0_30px_60px_-15px_rgba(255,255,255,0.5)] transition-all duration-500 flex items-center gap-3 group">
