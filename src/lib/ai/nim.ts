  ...
  const handleAI = async (action: AIAction) => {
    ...
    try {
      ...
      if (action === 'summarize') {
        ...
        // Aggressively search the raw JSON for the image or asset URL
        ...
        // Auto-fix raw base64 if it's missing the data tag
        ...
        if (!safePreview && !safeAsset) {
          setAiState('fallback');
          console.error("Failed to extract image. Raw NVIDIA payload was:", rawObj);
          toast.error('Generation completed but no preview URL was returned. Check console.');
        } else {
